...
namespace WingmanInspector {
    ...
    public class WingmanContainer {
        ...
        private List<ulong> selectedCompIds;
        private List<ulong> validCompIds = new List<ulong>();
        private List<ulong> prevValidCompIds = new List<ulong>();
        ...
        private void DrawWingmanGui() {
            ...
            for (int i = 0; i < comps.Count; i++) {
                ...
                validCompIds.Add(EntityId.ToULong(comps[i].GetEntityId()));
            }
            ...
            foreach (ulong validCompId in validCompIds) {
        ...
        private void DrawPreviewScrollView(List<Rect> placementRects, List<Component> comps, Rect innerScrollRect, Rect outerScrollRect) {
            ...
            { 
                const ulong allButtonId = ulong.MaxValue;
            ...
            for (int i = 0; i < comps.Count; i++) {
                ...
                ulong compId = EntityId.ToULong(comp.GetEntityId());
        ...
        private void OnButtonToggleOn(int compIndex) {
            ulong compId = ComponentIdFromIndex(compIndex);
        ...
        private void AddRangeToSelected(int compIndex) {
            ...
            for (int i = min; i <= max; i++) {
                ulong id = ComponentIdFromIndex(i);
        ...
        private List<Component> GetComponentsFromSelection() {
            ...
            foreach (ulong compId in selectedCompIds) {
        ...
        private DragAndDropVisualMode HierarchyDropHandler(EntityId dropTargetEntityId, HierarchyDropFlags dropMode, Transform parentForDraggedObjects, bool perform) {
            bool hasDropTarget = dropTargetEntityId.IsValid();
            bool copying = dropMode == HierarchyDropFlags.DropUpon && hasDropTarget;
            bool creating = !hasDropTarget || dropMode == HierarchyDropFlags.DropBetween || dropMode == HierarchyDropFlags.None;
            ...
            if (copying && EditorUtility.EntityIdToObject(dropTargetEntityId) is GameObject gameObject) {
        ...
        private bool CompareComponentIds(List<ulong> list0, List<ulong> list1) {
        ...
        private void UpdateComponentVisibility() {
            ...
            for (int i = startIndex; i < editorListVisual.childCount; i++) {
                ...
                if (compFromIndex.TryGetValue(compIndex, out Component comp)) {
                    bool showComp = selectedCompIds.Count <= 0 || selectedCompIds.Contains(EntityId.ToULong(comp.GetEntityId()));
        ...
        private ulong ComponentIdFromIndex(int index) {
            return EntityId.ToULong(compFromIndex[index].GetEntityId());
        }

        private Component ComponentFromId(ulong compId) {
            int index = 0;
