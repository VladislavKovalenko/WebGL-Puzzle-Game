    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                // -------------------------------------------------------
                // 4. Дым (оригинальная формула)
                // -------------------------------------------------------
                float2 smokeUV = IN.uv * _SmokeScale;
                float smoke = smokeDensityFunc(smokeUV, _Time.y);

                // -------------------------------------------------------
                // 5. Плотность тумана (оригинальная формула)
                // -------------------------------------------------------
                float fogDensity = flashlightMask * (1.0 - smoke * _SmokeDensity);

                // -------------------------------------------------------
                // 6. Оригинальное смешивание тумана (как в v1)
                // -------------------------------------------------------
                half4 col = _FogColor;
                col.a *= fogDensity;

                // -------------------------------------------------------
                // 7. Цвет фонарика — добавляется отдельным слоем
                //    Виден только внутри круга (где fogDensity низкий)
                //    Не трогает и не размывает дым
                // -------------------------------------------------------
                half3 lightTint = _LightColor.rgb * _LightIntensity * lightMask * (1.0 - fogDensity);
                col.rgb += lightTint;
